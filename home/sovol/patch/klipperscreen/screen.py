...
from panels.plr import PlrPanel
...
class KlipperScreen(Gtk.Window):
    ...
    def __init__(self, args):
        ...
        self.plr_bool = self.read_varibles_cfg_value('was_interrupted')
        ...
        if self.plr_bool == "True":
            self.plr = PlrPanel(self, title="plr")
    ...
    def move(self, widget, axis, direction):
        ...
        speed = None if self.base_panel.ks_printer_cfg is None else self.base_panel.ks_printer_cfg.getint(config_key, None)
        if speed is None:
            speed = self._config.get_config()['main'].getint(config_key, 20)
        speed = 60 * max(1, speed)
        script = f"{KlippyGcodes.MOVE_RELATIVE}\nG0 {axis}{dist} F{speed}"
        self.base_panel._screen._send_action(widget, "printer.gcode.script", {"script": script})
        if self.base_panel._printer.get_stat("gcode_move", "absolute_coordinates"):
            self.base_panel._screen._ws.klippy.gcode_script("G90")

    # tiaopin ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    def screws_tilt_calculate(self, widget):
        # widget.set_sensitive(False)
        self.base_panel.home()
        self.response_count = 0
        # self.buttons['screws'].set_sensitive(False)
        self.base_panel._screen._send_action(widget, "printer.gcode.script", {"script": "SCREWS_TILT_CALCULATE"})

    # start print ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    def confirm_print_response(self, filename):
        logging.info(f"Starting print: {filename}")
        self.base_panel._screen._ws.klippy.print_start(filename)
