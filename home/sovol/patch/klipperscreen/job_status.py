...
class Panel(ScreenPanel):
    ...
    def update_time_left(self):
        ...
        self.syncValueToSerial("est", elapsed_label)
        self.syncValueToSerial("dur", self.format_eta(estimated, print_duration))

    # write data to serial screen ++++++++++++++++++++++++++++++++++++++++++++++++++
    def syncValueToSerial(self, dev, value):
        progress = ""
        label = dev
        if dev == "progress":
            progress = int(value * 100)
            result = self.seri.write(f"{label}.val={progress}".encode("GB2312"))
        elif dev == "percent":
            progress = f"{value * 100:.0f}%"
            result = self.seri.write(f"{label}.txt=\"{progress}\"".encode("GB2312"))
        else:
            result = self.seri.write(f"{label}.txt=\"{value}\"".encode("GB2312"))
    ...
    def set_state(self, state, msg=""):
        if state == "printing":
            ...
            # cancelled ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            result = self.seri.write(f"page print".encode("GB2312"))
            self.seri.write(bytes.fromhex('ff ff ff'))
        elif state == "paused":
