...
class ScreenPanel:
    ...
    def __init__(self, screen, title, **kwargs):
        ...
        #self.seri = serial.Serial('/dev/ttyS3',9600,8,'N')
        #if self.seri.isOpen():
        #    print("serial open success!!!")
        #else:
        #    print("serial open failed!!!")

        #self.mythread = Thread(target=self.run_forever, daemon=True)
        #logging.debug("Starting websocket thread")
        #self.mythread.start()

    # recv data from serial screen
    def run_forever(self):
        while True:
            time.sleep(0.1)
            count = self.seri.inWaiting()
            data = self.seri.read(count)
            if count >= 4:
                # tuple_data = struct.unpack("i", data)
                key_code = data[0]
                distanceArray = [10, 1, 0.1]
                if data[1] >= 1 and data[1] <= 3:
                    distance = distanceArray[data[1] - 1]
                    self._screen.setDistance(distance)
                if key_code == 1:
                    self._screen.move(None, 'Z', '+')
                elif key_code == 2:
                    self._screen.move(None, 'Z', '-')
                elif key_code == 3:
                    self._screen.move(None, 'Y', '+')
                elif key_code == 4:
                    self._screen.move(None, 'Y', '-')
                elif key_code == 5:
                    self._screen.move(None, 'X', '+')
                elif key_code == 6:
                    self._screen.move(None, 'X', '-')
                elif key_code == 0x12: # home
                    self._screen._send_action(None, "printer.gcode.script", {"script": "G28"})
                elif key_code == 0x13: # leveling
                    self._screen.screws_tilt_calculate(None)
                elif key_code == 0x20: # stop
                    self._screen._ws.klippy.print_cancel()
                elif key_code == 0x21: # pause
                    self._screen._ws.klippy.print_pause()
                elif key_code == 0x22: # resume
                    self._screen._ws.klippy.print_resume()
                elif key_code == 0x23: # disable motor
                    script = {"script": "M18"}
                    self._screen._confirm_send_action(
                        None,
                        ("Are you sure you wish to disable motors?"),
                        "printer.gcode.script", script
                    )
                elif key_code == 0x2E:
                    temperatureStr = data.decode("GB2312")
                    temp = temperatureStr.split("/")[1]
                    self._screen._ws.klippy.set_ext_temp(temp)
                elif key_code == 0x2F:
                    temperatureStr = data.decode("GB2312")
                    temp = temperatureStr.split("/")[1]
                    self._screen._ws.klippy.set_bed_temp(temp)
                elif key_code >= 0xf1 and key_code <= 0xf5: # refresh
                    print(key_code)
                    page = key_code - 0xf0 - 1
                    self.sendFilesToScreen(page)
                elif key_code == 0x3C:
                    filename = data.decode("GB2312").split("<")[1]
                    print("++", filename, "++")
                    self._screen.confirm_print_response(filename)
    ...
    # send gcode file to serial screen ++++++++++++++++++++++++++++++++++++++++++++++
    def sendFilesToScreen(self, page):
        count = len(self._screen.files.filelist)
        lastIndex = page * 4 + 4
        newFileList = self._screen.files.filelist[page*4: lastIndex]
        if (lastIndex > count):
            if lastIndex - count < 4:
                tempArr = [""]*(lastIndex - count)
            else:
                tempArr = [""]*4
            newFileList += tempArr
        for i, item in enumerate(newFileList):
            result = self.seri.write(f"b{i+1}.txt=\"{item}\"".encode("GB2312"))
            self.seri.write(bytes.fromhex('ff ff ff'))
    
    # write data to serial screen ++++++++++++++++++++++++++++++++++++++++++++++++++
    def syncTempToSerial(self, dev, temp, target):
        if target is None:
            tempItem = f"{dev}:{int(temp)}"
        else:
            tempItem = f"{dev}:{int(temp)}/{int(target)}"
        label = ""
        if dev == "extruder":
            label = "t0"
        elif dev == "heater_bed":
            label = "t1"
        result = self.seri.write(f"{label}.txt=\"{tempItem}\"".encode("GB2312"))
        self.seri.write(bytes.fromhex('ff ff ff'))
    ...
    def update_temp(self, dev, temp, target, power, lines=1):
        ...
        #self.syncTempToSerial(dev, temp, target)

        speed = self._printer.get_fan_speed()
        if dev == "extruder":
            print("+++++++++++++++++", temp, ",", speed, "-------------------")
