...
class BasePanel(ScreenPanel):
    def __init__(self, screen, title):
        ...
        self.update_time()
        self.current_pos_z = self.previous_pos_z = 0.0
    ...
    def process_update(self, action, data):
        ...
        if "gcode_move" in data:
            with contextlib.suppress(KeyError):
                if (data['gcode_move']['gcode_position'][2]) != self.current_pos_z:
                    self.previous_pos_z = self.current_pos_z
                    self.current_pos_z = (data['gcode_move']['gcode_position'][2])
                    self.modify_cfg_value('power_resume_z', str(round(float(self.current_pos_z), 2)))
                    # logging.info(f"更新高度: {str(round(float(self.current_pos_z), 2))}")
